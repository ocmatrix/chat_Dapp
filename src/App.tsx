...
const App: React.FC = () => {
  ...
  const handleLogout = () => {
    ...
    setMyPeerId('');
    // Note: We do NOT clear messages here so they persist. 
    // If you want to clear them on logout, uncomment next line:
    // setMessages([]); 
    setTargetPeerId('');
  ...
  const initializePeer = (id: string) => {
    ...
    // In a fully custom prod env, you would swap this for a GunDB or Waku signaling layer.
    ...
    peer.on('call', (call) => {
      // Determine call type based on metadata if available, or assume video
      // For simplicity in this demo, we ask the user.
  ...
  const handleConnection = (conn: DataConnection) => {
    ...
    conn.on('data', (data: any) => {
      ...
      // Trigger AI suggestion on incoming TEXT messages only
      if (msg.type === 'text') {
  ...
  return (
    <div className="flex flex-col h-screen bg-crypto-dark text-white overflow-hidden font-sans">
      ...
      <header className="flex items-center justify-between p-4 bg-crypto-surface/90 backdrop-blur-md border-b border-gray-700/50 z-10 sticky top-0">
        <div className="flex flex-col">
           ...
           <div className="flex items-center gap-2 text-xs text-gray-400 mt-0.5">
             <span className={`w-2 h-2 rounded-full ${status === ConnectionStatus.CONNECTED ? 'bg-crypto-success shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-red-500'}`}></span>
        ...
        <div className="flex gap-2">
             <button 
                ...
                className="p-2 bg-gray-800 text-gray-300 rounded-full hover:bg-gray-700 hover:text-white transition-all active:scale-95"
            >
            ...
            <button 
                onClick={() => startCall(false)} // Audio Call
                disabled={!targetPeerId || status !== ConnectionStatus.CONNECTED}
                className="p-2 bg-gray-800 text-crypto-success rounded-full hover:bg-gray-700 transition-all border border-gray-700 disabled:opacity-30 active:scale-95"
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button 
                onClick={() => startCall(true)} // Video Call
                disabled={!targetPeerId || status !== ConnectionStatus.CONNECTED}
                className="p-2 bg-gray-800 text-crypto-primary rounded-full hover:bg-gray-700 transition-all border border-gray-700 disabled:opacity-30 active:scale-95"
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button 
                ...
                className="p-2 ml-1 bg-gray-800 text-red-400 rounded-full hover:bg-gray-700 hover:text-red-300 transition-all active:scale-95 border border-red-900/30"
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Manual Connection Bar (if not connected) */}
      {!connRef.current && (
          <div className="p-4 bg-gray-900 border-b border-gray-800 animate-fade-in">
              <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 bg-crypto-surface border border-gray-700 rounded-lg px-3 py-3 text-sm focus:outline-none focus:border-crypto-primary placeholder-gray-600 transition-colors"
                    value={targetPeerId}
                    onChange={(e) => setTargetPeerId(e.target.value.toLowerCase())}
                  />
                  <button 
                    onClick={() => connectToPeer(targetPeerId)}
                    className="bg-crypto-primary px-5 py-2 rounded-lg text-sm font-bold hover:bg-blue-600 shadow-lg shadow-blue-900/20 active:scale-95 transition-all"
                  >
                      Connect
                  </button>
              </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-gradient-to-b from-crypto-dark to-gray-900">
        {messages.length === 0 && (
            <div className="text-center text-gray-500 mt-20 text-sm flex flex-col items-center">
                ...
                <p className="text-xs mt-2 text-gray-600">Messages are end-to-end encrypted and not stored on any server.</p>
        ...
        {messages.map((msg) => (
          ...
          >
            {msg.type === 'system' ? (
                ...
                <div className={`max-w-[85%] rounded-2xl px-4 py-3 shadow-md relative group ${
                    msg.sender === 'Me' 
                        ? 'bg-crypto-primary text-white rounded-br-none' 
                        : msg.isAi 
                            ? 'bg-gray-800 border border-purple-500/30 text-gray-100 rounded-bl-none'
                            : 'bg-crypto-surface text-gray-200 rounded-bl-none border border-gray-700'
                }`}>
                    {msg.isAi && (
                        <div className="flex items-center gap-1 mb-1">
                             <svg className="w-3 h-3 text-purple-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm0 18a8 8 0 1 1 8-8 8 8 0 0 1-8 8z"/></svg>
      ...
      {aiSuggestion && (
        <div className="px-4 pb-2 bg-gradient-to-t from-crypto-dark to-transparent">
            <button 
                onClick={() => { setInputMsg(aiSuggestion); setAiSuggestion(null); }}
                className="flex items-center gap-2 text-xs bg-crypto-accent/20 text-crypto-accent border border-crypto-accent/50 px-4 py-2 rounded-full hover:bg-crypto-accent/30 transition-all active:scale-95 shadow-lg shadow-purple-900/20 backdrop-blur-sm"
            >
      ...
      <div className="p-3 bg-crypto-surface border-t border-gray-800 safe-area-bottom">
        <div className="flex items-end gap-2 bg-gray-900/50 p-1.5 rounded-3xl border border-gray-700/50">
            {/* AI Toggle */}
            <button 
                className={`p-2 rounded-full transition-colors mb-0.5 ${inputMsg.startsWith('/ai ') ? 'text-purple-400 bg-purple-900/20' : 'text-gray-400 hover:text-crypto-primary'}`} 
                onClick={() => setInputMsg(prev => prev.startsWith('/ai ') ? prev.replace('/ai ', '') : '/ai ' + prev)}
            >
            ...
            <textarea
                ...
                className="flex-1 bg-transparent text-white max-h-24 py-2.5 px-2 focus:outline-none text-sm resize-none disabled:opacity-50"
                style={{ minHeight: '40px' }}
            />
            
            {/* Record / Send Button Logic */}
            {inputMsg.trim() ? (
                <button 
                    onClick={sendMessage}
                    className="p-2.5 bg-crypto-primary rounded-full text-white hover:bg-blue-600 shadow-lg mb-0.5 transition-transform active:scale-90"
                >
                ...
                <button 
                    ...
                    className={`p-2.5 rounded-full text-white shadow-lg mb-0.5 transition-all active:scale-90 ${isRecording ? 'bg-red-500 animate-pulse ring-4 ring-red-500/30' : 'bg-gray-700 hover:bg-gray-600'}`}
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
      ...
      <VideoCall 
        ...
        onEndCall={() => {
            peerRef.current?.connections[targetPeerId]?.forEach((c: any) => c.close());
            endCall();
        }} 
      />
    </div>
